...
def Flood_fill_8(x, y,oldColor):
    ...
    if (color == oldColor ):
        ...
        glEnd()
        glFlush()
        Flood_fill_8(x+5, y,oldColor)
        Flood_fill_8(x-5, y,oldColor)
        Flood_fill_8(x, y+5,oldColor)
        Flood_fill_8(x, y-5,oldColor)
        Flood_fill_8(x-5, y-5,oldColor)
        Flood_fill_8(x-5, y+5,oldColor)
        Flood_fill_8(x+5, y-5,oldColor)
        Flood_fill_8(x+5, y+5,oldColor)
...
def readinput(x,y):
    ...
    oldColor =  b'\x00\x00\x00'
    # Flood_fill_8(x,y,oldColor)
    color = glReadPixels(x, y, 1.0, 1.0, GL_RGB, GL_UNSIGNED_BYTE, None)
    if (color == oldColor ):
        glColor3f(0.0,1.0,0.0)
        glPointSize(4.0)
...
def RectLineBresenham(x0=22, y0=11, l=200, b=250):
    x1 = x0
    y1 = y0
